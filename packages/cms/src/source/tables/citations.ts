...
import { type Database, touchCitation } from '../../db';

const hooks: FileHooks = {
    async initialize(db, _filename, _frontmatter, contents) {
        initialize(db, contents);
    },
    async hmr(db, _filename, _frontmatter, contents) {
        initialize(db, contents);
    },
};

function initialize(db: Database, file: string) {
    const bibFile = bibtex.parseBibFile(file);

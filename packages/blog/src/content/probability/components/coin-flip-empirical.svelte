...
{#snippet success()}
    <button
        ...
        onclick={() => (pEst = lib.bernoulli_sample_statistic!(p, samples))}>Randomly sample</button
    >
    <label>
    ...
    <svg viewBox="0 0 {scale} {scale}">
        ...
        <text x={0.3 * scale} y={0.95 * scale} text-anchor="middle" fill="currentColor">Heads</text>
        <text x={0.7 * scale} y={0.95 * scale} text-anchor="middle" fill="currentColor">Tails</text>
...
{#snippet failure()}
    <p>The library unit <code>bernoulli_sample_statistic</code> failed to load.</p>
{/snippet}

<NeedJavascript
    isLoading={lib.loading}
    isSuccess={typeof lib.bernoulli_sample_statistic != 'undefined'}
    {success}
    {failure}
></NeedJavascript>
